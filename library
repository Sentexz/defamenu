...
function Menu.DrawScrollbar(x, startY, visibleHeight, selectedIndex, totalItems, isMainMenu, menuWidth)
    ...
    if isMainMenu then adjustedIndex = selectedIndex - 1 end
    ...
    for i, tab in ipairs(category.tabs) do
        ...
        if isSelected then
            ...
            for step = 0, gradientSteps - 1 do
                ...
                if stepY + actualStepHeight > maxY then actualStepHeight = maxY - stepY end
...
local function findNextNonSeparator(items, startIndex, direction)
    ...
    while attempts < maxAttempts do
        ...
        if items[index] and not items[index].isSeparator then return index end
...
function Menu.DrawItem(x, itemY, width, itemHeight, item, isSelected)
        ...
        if Menu.GradientType == 2 then
            ...
            for step = 0, gradientSteps - 1 do
                ...
                if actualStepWidth > 0 then
                    local stepGradientFactor = step / (gradientSteps - 1)
                    local easedFactor = stepGradientFactor < 0.5 and 4 * stepGradientFactor * stepGradientFactor * stepGradientFactor or 1 - math.pow(-2 * stepGradientFactor + 2, 3) / 2
                    ...
                    local alpha = 0.95
                    if step > gradientSteps - 20 then alpha = 0.95 * (1.0 - ((step - (gradientSteps - 20)) / 20)) end
                    if Susano and Susano.DrawRectFilled then
            ...
            for step = 0, gradientSteps - 1 do
                ...
                if actualStepHeight > 0 then
                    ...
                    local stepB = math.max(0, baseB - stepDarken)
                    local brightness = 1.0
                    if step < gradientSteps * 0.15 then brightness = 1.0 + (0.12 * (1.0 - step / (gradientSteps * 0.15))) end
                    stepR = math.min(1.0, stepR * brightness)
                    stepG = math.min(1.0, stepG * brightness)
                    stepB = math.min(1.0, stepB * brightness)
                    if Susano and Susano.DrawRectFilled then
    ...
    if item.type == "toggle" then
        ...
        local circleR, circleG, circleB = isGrayTheme and 1.0 or 0.0, isGrayTheme and 1.0 or 0.0, isGrayTheme and 1.0 or 0.0
        if Susano and Susano.DrawRectFilled then
        ...
        if item.hasSlider then
            ...
            if percent > 0 then
                if Susano and Susano.DrawRectFilled then
                    ...
                    accentR, accentG, accentB = math.min(1.0, accentR), math.min(1.0, accentG), math.min(1.0, accentB)
                    ...
                    Menu.DrawRoundedRect(sliderX, sliderY, sliderWidth * percent, sliderHeight, accentR, accentG, accentB, 255, 3.0)
                end
            end
            ...
            local valueText = (item.name == "Freecam") and string.format("%.1f", currentValue) or string.format("%.1f", currentValue)
    ...
    elseif item.type == "toggle_selector" then
        ...
        if item.options then
            ...
            Menu.DrawText(selectorX, textY, "<", selectorSize, Menu.Colors.TextWhite.r / 255.0 * 0.8, Menu.Colors.TextWhite.g / 255.0 * 0.8, Menu.Colors.TextWhite.b / 255.0 * 0.8, 0.8)
            ...
            Menu.DrawText(selectorX + leftArrowWidth + optionWidth + (5 * scale), textY, ">", 16, Menu.Colors.TextWhite.r / 255.0 * 0.8, Menu.Colors.TextWhite.g / 255.0 * 0.8, Menu.Colors.TextWhite.b / 255.0 * 0.8, 0.8)
        end
    elseif item.type == "slider" then
        ...
        local thumbY = itemY + (itemHeight / 2) - (thumbSize / 2)
        if Susano and Susano.DrawRectFilled then
            Susano.DrawRectFilled(thumbX, thumbY, thumbSize, thumbSize, 1.0, 1.0, 1.0, 1.0, 5.0 * scale)
        else
            Menu.DrawRoundedRect(thumbX, thumbY, thumbSize, thumbSize, 255, 255, 255, 255, 5.0 * scale)
        end
    ...
    elseif item.type == "selector" and item.options then
        ...
        for _, name in ipairs(wardrobeItemNames) do if item.name == name then isWardrobeSelector = true break end end
        if isWardrobeSelector then
            ...
            local selectorX = x + width - selectorWidth - (16 * scale)
            Menu.DrawText(selectorX, textY, "<", 17, Menu.Colors.TextWhite.r / 255.0 * 0.8, Menu.Colors.TextWhite.g / 255.0 * 0.8, Menu.Colors.TextWhite.b / 255.0 * 0.8, 0.8)
            local leftArrowWidth = 0
            if Susano and Susano.GetTextWidth then
                leftArrowWidth = Susano.GetTextWidth("< ", selectorSize)
            else
            ...
            Menu.DrawText(selectorX + leftArrowWidth, textY, selectedOption, 17, 1.0, 1.0, 1.0, 1.0)
            local optionWidth = 0
            if Susano and Susano.GetTextWidth then
                optionWidth = Susano.GetTextWidth(selectedOption, selectorSize)
            else
            ...
            Menu.DrawText(selectorX + leftArrowWidth + optionWidth + (5 * scale), textY, ">", 17, Menu.Colors.TextWhite.r / 255.0 * 0.8, Menu.Colors.TextWhite.g / 255.0 * 0.8, Menu.Colors.TextWhite.b / 255.0 * 0.8, 0.8)
        end
    end
end
...
function Menu.DrawCategories()
    if Menu.OpenedCategory then
        ...
        if currentTab and currentTab.items then
            ...
            for _, item in ipairs(currentTab.items) do if not item.isSeparator then nonSeparatorCount = nonSeparatorCount + 1 end end
    ...
    for step = 0, gradientSteps - 1 do
        ...
        local brightness = 1.0
        if step < gradientSteps * 0.3 then brightness = 1.0 + (0.2 * (1.0 - step / (gradientSteps * 0.3))) end
    ...
    if Menu.TopLevelTabs then
        ...
        for i, tab in ipairs(Menu.TopLevelTabs) do
            ...
            if isSelected then
                if not Menu.TopTabSelectorX then Menu.TopTabSelectorX = tabX; Menu.TopTabSelectorWidth = tabWidth end
                local smoothSpeed = Menu.SmoothFactor
                ...
                for step = 0, gradientSteps - 1 do
                    ...
                    local brightness = 1.0
                    if step < gradientSteps * 0.2 then brightness = 1.0 + (0.1 * (1.0 - step / (gradientSteps * 0.2))) end
                    local stepR = math.min(1.0, baseR * brightness)
                    local stepG = math.min(1.0, baseG * brightness)
                    local stepB = math.min(1.0, baseB * brightness)
                    if Susano and Susano.DrawRectFilled then
            ...
            if not isSelected then r, g, b = 150, 150, 150 end
    ...
    for displayIndex = 1, math.min(maxVisible, totalCategories) do
        ...
        if categoryIndex <= #Menu.Categories then
            ...
            if isSelected then
                if Menu.CategorySelectorY == 0 then Menu.CategorySelectorY = itemY end
                local smoothSpeed = Menu.SmoothFactor
                Menu.CategorySelectorY = Menu.CategorySelectorY + (itemY - Menu.CategorySelectorY) * smoothSpeed
                if math.abs(Menu.CategorySelectorY - itemY) < 0.5 then Menu.CategorySelectorY = itemY end
                ...
                if Menu.GradientType == 2 then
                    ...
                    for step = 0, gradientSteps - 1 do
                        ...
                        if actualStepWidth > 0 then
                            ...
                            local stepB = math.max(0, baseB - stepDarken)
                            local brightness = 1.0
                            if step < gradientSteps * 0.1 then brightness = 1.0 + (0.15 * (1.0 - step / (gradientSteps * 0.1))) end
                            stepR = math.min(1.0, stepR * brightness)
                            stepG = math.min(1.0, stepG * brightness)
                            stepB = math.min(1.0, stepB * brightness)
                            local alpha = 0.95
...
function Menu.DrawLoadingBar(alpha)
    ...
    local elapsedTime = currentTime - (Menu.LoadingStartTime or currentTime)
    local loadingText = ""
    if elapsedTime < 1000 then loadingText = "Injecting"
    elseif elapsedTime < 2000 then loadingText = "Have Fun !"
    else loadingText = "Have Fun !"
    end
    if loadingText ~= "" then
        local textSize = 18
        local textWidth = 0
        if Susano and Susano.GetTextWidth then textWidth = Susano.GetTextWidth(loadingText, textSize) else textWidth = string.len(loadingText) * 10 end
    ...
    if Susano and Susano.GetTextWidth then percentTextWidth = Susano.GetTextWidth(percentText, percentTextSize) else percentTextWidth = string.len(percentText) * 9 end
...
function Menu.DrawFooter()
    ...
    if Susano and Susano.DrawRectFilled then
        Susano.DrawRectFilled(x, footerY, footerWidth, footerHeight, 0.0, 0.0, 0.0, 1.0, footerRounding)
    else
    ...
    local textWidth = 0
    if Susano and Susano.GetTextWidth then textWidth = Susano.GetTextWidth(footerText, scaledFooterSize) else textWidth = string.len(footerText) * 8 * scale end
    ...
    local posWidth = 0
    if Susano and Susano.GetTextWidth then posWidth = Susano.GetTextWidth(posText, scaledFooterSize) else posWidth = string.len(posText) * 8 * scale end
...
function Menu.DrawKeybindsInterface(alpha)
                ...
                if tab.items then
                    for _, item in ipairs(tab.items) do
                        if item.bindKey and item.bindKeyName and (item.type == "toggle" or item.type == "action") then
                            table.insert(activeBinds, { name = item.name, keyName = item.bindKeyName, isActive = (item.type == "toggle" and (item.value or false)) or nil })
                        end
                    end
                end
            end
    ...
    for _, bind in ipairs(activeBinds) do
        ...
        local textWidth = Susano and Susano.GetTextWidth and Susano.GetTextWidth(text, textSize) or (string.len(text) * 8)
    ...
    for i, bind in ipairs(activeBinds) do
        local text = bind.isActive ~= nil and (bind.name .. " (" .. bind.keyName .. ") [" .. (bind.isActive and "on" or "off") .. "]") or (bind.name .. " (" .. bind.keyName .. ")")
...
function Menu.DrawBackground()
    ...
    for i, seg in ipairs(segments) do
        if i == #segments then break end
        if seg.y >= itemsEndY then break end
        if seg.y < menuBarY then
            local offset = menuBarY - seg.y
            if offset >= seg.h then else
                seg = {y = menuBarY, h = seg.h - offset}
            end
        end
        if seg.y < menuBarY or seg.h <= 0 then else
            local segSteps = math.ceil(seg.h / 2)
            for j = 0, segSteps - 1 do
                local localY = j * 2
                local drawH = 2
                ...
                if currentY < menuBarY then
                    ...
                    if adjust >= drawH then else
                        currentY = menuBarY
                        drawH = drawH - adjust
                    end
                end
                if currentY >= menuBarEndY and currentY < itemsY then else
                    if currentY >= itemsEndY then break end
                    if currentY + drawH > itemsEndY then drawH = itemsEndY - currentY end
                    if drawH <= 0 then break end
                    local isTabArea = (currentY >= menuBarY and currentY < menuBarEndY)
                    local backgroundAlpha = 1.0
                    if not isTabArea then
                                ...
                                if cat.name == "Settings" and cat.tabs then
                                    for _, tab in ipairs(cat.tabs) do
                                        if tab.name == "General" and tab.items then
                                            for _, item in ipairs(tab.items) do
                                                if item.name == "Black Background" then blackBackgroundItem = item; break end
                                            end
                                        end
                                    end
                                end
                        ...
                        backgroundAlpha = (blackBackgroundItem and blackBackgroundItem.value == false) and 0.2 or 1.0
                    end
                    if Susano and Susano.DrawRectFilled then
    ...
    if Menu.ShowSnowflakes then
        for _, p in ipairs(Menu.Particles) do
            ...
            for i, seg in ipairs(segments) do
                if i == #segments then break end
                if pY >= seg.y and pY <= seg.y + seg.h then isVisible = true; break end
            end
            if isVisible then
...
function Menu.Render()
    ...
    if Menu.IsLoading then Menu.LoadingBarAlpha = math.min(1.0, Menu.LoadingBarAlpha + animSpeed)
    else Menu.LoadingBarAlpha = math.max(0.0, Menu.LoadingBarAlpha - animSpeed) end
    if Menu.SelectingKey or Menu.SelectingBind then Menu.KeySelectorAlpha = math.min(1.0, Menu.KeySelectorAlpha + animSpeed)
    else Menu.KeySelectorAlpha = math.max(0.0, Menu.KeySelectorAlpha - animSpeed) end
    if Menu.ShowKeybinds then Menu.KeybindsInterfaceAlpha = math.min(1.0, Menu.KeybindsInterfaceAlpha + animSpeed)
    else Menu.KeybindsInterfaceAlpha = math.max(0.0, Menu.KeybindsInterfaceAlpha - animSpeed) end
    Susano.BeginFrame()
    if Menu.KeybindsInterfaceAlpha > 0 then Menu.DrawKeybindsInterface(Menu.KeybindsInterfaceAlpha) end
    if Menu.Visible then
        if Menu.EditorMode and Susano and Susano.EnableOverlay then Susano.EnableOverlay(true)
        elseif not Menu.EditorMode and Susano and Susano.EnableOverlay then Susano.EnableOverlay(false) end
        Menu.DrawBackground()
        Menu.DrawHeader()
        Menu.DrawCategories()
        Menu.DrawFooter()
...
function Menu.HandleInput()
    ...
    if Menu.SelectingBind then
        ...
        if Menu.IsKeyJustPressed(0x0D) then
            if Menu.BindingKey and Menu.BindingItem then
                ...
                print("Bind set for " .. (Menu.BindingItem.name or "option") .. ": " .. tostring(Menu.BindingKeyName))
            end
            Menu.SelectingBind = false
            Menu.BindingItem = nil
            Menu.BindingKey = nil
        ...
        local keysToCheck = {0x41,0x42,0x43,0x44,0x45,0x46,0x47,0x48,0x49,0x4A,0x4B,0x4C,0x4D,0x4E,0x4F,0x50,0x51,0x52,0x53,0x54,0x55,0x56,0x57,0x58,0x59,0x5A,0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x20,0x1B,0x08,0x09,0x10,0x11,0x12,0x25,0x26,0x27,0x28,0x70,0x71,0x72,0x73,0x74,0x75,0x76,0x77,0x78,0x79,0x7A,0x7B}
    ...
    if Susano and Susano.GetAsyncKeyState then
        ...
        local keyPressed = (pressed == true) or (down == true and not wasDown)
        Menu.KeyStates[toggleKeyCode] = (down == true)
        if keyPressed then
    ...
    if Menu.EditorMode then
        ...
        if Susano and Susano.GetCursorPos and Susano.GetAsyncKeyState then
            ...
            if Susano.GetAsyncKeyState then
                ...
                if lmbDown == true or lmbDown == 1 then leftMouseDown = true end
            end
            if not leftMouseDown and (IsControlPressed or IsDisabledControlPressed) then
                if IsDisabledControlPressed and IsDisabledControlPressed(0, 24) then leftMouseDown = true
                elseif IsControlPressed and IsControlPressed(0, 24) then leftMouseDown = true end
            end
            ...
            local isOverMenu = (mouseX >= menuX and mouseX <= menuX + menuWidth and mouseY >= menuY and mouseY <= menuY + totalHeight)
        ...
        if Susano and Susano.GetAsyncKeyState then
            ...
            if upDown == true then Menu.Position.y = math.max(0, Menu.Position.y - moveSpeed) end
            if downDown == true then Menu.Position.y = math.min(screenH - 200, Menu.Position.y + moveSpeed) end
            if leftDown == true then Menu.Position.x = math.max(0, Menu.Position.x - moveSpeed) end
            if rightDown == true then Menu.Position.x = math.min(screenW - Menu.Position.width, Menu.Position.x + moveSpeed) end
            if Menu.IsKeyJustPressed(0x0D) then
                local currentTab = nil
                if Menu.OpenedCategory then
                    local category = Menu.Categories[Menu.OpenedCategory]
                    if category and category.hasTabs and category.tabs then currentTab = category.tabs[Menu.CurrentTab] end
                end
                if currentTab and currentTab.items then
    ...
    if Menu.OpenedCategory then
        ...
        if currentTab and currentTab.items then
            if Susano and Susano.GetAsyncKeyState then
                ...
                Menu.KeyStates[0x26] = (upDown == true)
                Menu.KeyStates[0x28] = (downDown == true)
                Menu.KeyStates[0x41] = (aDown == true)
                Menu.KeyStates[0x45] = (eDown == true)
                Menu.KeyStates[0x51] = (qDown == true)
                Menu.KeyStates[0x08] = (backDown == true)
                Menu.KeyStates[0x25] = (leftDown == true)
                Menu.KeyStates[0x27] = (rightDown == true)
                Menu.KeyStates[0x78] = (f9Down == true)
                ...
                elseif (downPressed == true) or (downDown == true and not downWasDown) then
                    Menu.CurrentItem = findNextNonSeparator(currentTab.items, Menu.CurrentItem, 1)
                elseif (aPressed == true) or (aDown == true and not aWasDown) or (qPressed == true) or (qDown == true and not qWasDown) then
                    if Menu.CurrentTab > 1 then
                        Menu.CurrentTab = Menu.CurrentTab - 1
                        local newTab = category.tabs[Menu.CurrentTab]
                        if newTab and newTab.items then Menu.CurrentItem = findNextNonSeparator(newTab.items, 0, 1) else Menu.CurrentItem = 1 end
                        Menu.ItemScrollOffset = 0
                    ...
                    if Menu.CurrentItem > 0 and Menu.CurrentItem <= #currentTab.items then
                        ...
                        if selectedItem then
                            if selectedItem.type == "slider" then
                                local step = selectedItem.step or 1.0
                                selectedItem.value = math.max(selectedItem.min or 0.0, (selectedItem.value or selectedItem.min or 0.0) - step)
                                if selectedItem.name == "Smooth Menu" then Menu.SmoothFactor = selectedItem.value / 100.0
                                elseif selectedItem.name == "Menu Size" then Menu.Scale = selectedItem.value / 100.0 end
                            ...
                            elseif selectedItem.type == "toggle_selector" then
                                ...
                                if selectedItem.options and #selectedItem.options > 0 then
                                    ...
                                    if currentIndex < 1 then currentIndex = #selectedItem.options end
                                end
                            ...
                            elseif selectedItem.type == "selector" then
                                ...
                                local isWardrobeSelector = false
                                local wardrobeItemNames = {"Hat", "Mask", "Glasses", "Torso", "Tshirt", "Pants", "Shoes"}
                                for _, name in ipairs(wardrobeItemNames) do if selectedItem.name == name then isWardrobeSelector = true break end end
                                if isWardrobeSelector then
                                    currentIndex = math.max(1, currentIndex - 1)
                                else
                                    if selectedItem.options and #selectedItem.options > 0 then
                                        currentIndex = currentIndex - 1
                                        if currentIndex < 1 then currentIndex = #selectedItem.options
                                    end
                                end
                                selectedItem.selected = currentIndex
                                if selectedItem.name == "Menu Theme" and selectedItem.options then Menu.ApplyTheme(selectedItem.options[currentIndex])
                                elseif selectedItem.name == "Gradient" and selectedItem.options then Menu.GradientType = tonumber(selectedItem.options[currentIndex]) or 1
                                elseif selectedItem.name == "Scroll Bar Position" and selectedItem.options then
                                    Menu.ScrollbarPosition = (selectedItem.options[currentIndex] == "Left") and 1 or 2
                                end
                            end
                        end
                    end
                    ...
                    if Menu.CurrentItem > 0 and Menu.CurrentItem <= #currentTab.items then
                        ...
                        if selectedItem then
                            ...
                            elseif selectedItem.type == "toggle_selector" then
                                ...
                                if selectedItem.options and #selectedItem.options > 0 then
                                    currentIndex = currentIndex + 1
                                    if currentIndex > #selectedItem.options then currentIndex = 1 end
                                end
                                selectedItem.selected = currentIndex
                            elseif selectedItem.type == "selector" then
                                local currentIndex = selectedItem.selected or 1
            ...
            if Menu.IsKeyJustPressed(0x0D) then
                ...
                if item and not item.isSeparator then
                    if item.type == "toggle" or item.type == "toggle_selector" then
                        item.value = not item.value
                        if item.name == "Show Menu Keybinds" then Menu.ShowKeybinds = item.value
                        elseif item.name == "Editor Mode" then Menu.EditorMode = item.value
                        elseif item.name == "Flakes" then Menu.ShowSnowflakes = item.value end
                        if item.onClick then item.onClick(item.value) end
                    elseif item.type == "action" then
...
function Menu.OpenInput(title, subtitle, callback)
    if type(subtitle) == "function" then callback = subtitle; subtitle = "Enter text below" end
...
function Menu.DrawInputWindow()
    ...
    local titleSize = 20
    local titleWidth = 0
    if Susano and Susano.GetTextWidth then titleWidth = Susano.GetTextWidth(titleText, titleSize) else titleWidth = string.len(titleText) * 10 end
    ...
    if math.floor(GetGameTimer() / 500) % 2 == 0 then displayText = displayText .. "|" end
    local maxDisplayChars = 30
    if string.len(displayText) > maxDisplayChars then displayText = "..." .. string.sub(displayText, -maxDisplayChars) end
    ...
    if Susano and Susano.GetAsyncKeyState then
        ...
        if Menu.IsKeyJustPressed(0x08) and string.len(Menu.InputText) > 0 then Menu.InputText = string.sub(Menu.InputText, 1, -2) end
        if Menu.IsKeyJustPressed(0x1B) then Menu.InputOpen = false end
        local shiftPressed = false
        if Susano.GetAsyncKeyState(0x10) or Susano.GetAsyncKeyState(0xA0) or Susano.GetAsyncKeyState(0xA1) then shiftPressed = true end
        for i = 0x41, 0x5A do
        ...
        if Menu.IsKeyJustPressed(0xBD) then Menu.InputText = Menu.InputText .. (shiftPressed and "_" or "-") end
    end
end
...
Menu.TopLevelTabs = {
    { name = "Menu", categories = {
        { name = "Main", icon = "🏠", hasTabs = true, tabs = {
            { name = "General", items = {
                { name = "Godmode", type = "toggle", value = false, onClick = function(val) print("Godmode:", val) end },
                { name = "Noclip", type = "toggle", value = false },
                { name = "Fast Run", type = "toggle", value = false },
                { name = "", isSeparator = true, separatorText = "Settings" },
                { name = "Menu Theme", type = "selector", options = {"Purple", "Red", "Gray", "pink"}, selected = 1, onClick = function(idx,opt) Menu.ApplyTheme(opt) end },
                { name = "Flakes", type = "toggle", value = false, onClick = function(val) Menu.ShowSnowflakes = val end },
                { name = "Black Background", type = "toggle", value = true },
            } }
        } }
    } },
    { name = "Player", categories = {
        { name = "Self", icon = "👤", hasTabs = true, tabs = {
            { name = "Self", items = {
                { name = "Godmode", type = "toggle", value = false, onClick = function(val) print("Godmode:", val) end },
                { name = "Semi Godmode", type = "toggle", value = false },
                { name = "Anti Headshot", type = "toggle", value = false },
                { name = "Max Health", type = "action", onClick = function() print("Max Health") end },
                { name = "Max Armor", type = "action", onClick = function() print("Max Armor") end },
            } }
        } }
    } }
}
